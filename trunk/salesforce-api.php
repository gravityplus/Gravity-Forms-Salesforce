...
use OAuth\Common\Service\SalesforceService;
use OAuth\Common\Http\Client\CurlClient;
use OAuth\Common\Storage\WordPressMemory as WordPressMemory;
use OAuth\Common\Consumer\Credentials;
use OAuth\Common\Http\Uri\Uri;

add_action('plugins_loaded', array('GFSalesforce', 'processSalesforceResponse'));
...
class GFSalesforce {
    ...
    public static function init(){
        global $pagenow;
        require_once(plugin_dir_path(__FILE__).'/edit-form.php');
    ...
    static public function processSalesforceResponse() {
        if (!empty($_GET['code']) && !empty($_GET['display']) && $_GET['display'] === 'page') {
            ...
            wp_redirect( admin_url('admin.php?page=gf_settings&addon=Salesforce') );
    ...
    static public function processErrorMessage($value='') {
        ...
        switch(true) {
            case strpos($value, 'invalid_grant'):
                $value = "This authorization request has expired.";
            break;
            case strpos($value, 'redirect_uri_mismatch'):
                $value = "redirect_uri mismatch with remote access application definition";
            break;
            ...
            case strpos($value, 'invalid_client'):
                $value = "Client consumer secret invalid";
            break;
        }
...
EOD;
    ...
    public static function settings_page(){
        ...
        <form method="post" action="<?php echo remove_query_arg('refresh'); ?>">
            ...
            <table class="form-table">
                <tr>
                    <th scope="row"><label for="gf_salesforce_debug"><?php _e("Salesforce Account", "gravity-forms-salesforce"); ?></label> </th>
                    <td>
                    <?php
                    ...
                    if(!empty($token)) {
                        ...
                        echo '<span class="gf_keystatus_valid_text">';
                        echo '<i class="fa fa-check gf_keystatus_valid"></i> ';
                        printf(__('Authorized connection to <code>%s</code> on %s'), str_replace('https://', '', $params['instance_url']), date_i18n('F d, Y H:i:m \G\M\T', substr($params['issued_at'], 0, 10)));
                        echo '</span>';
                        $url = wp_nonce_url(add_query_arg(array('cleartoken' => true)), 'cleartoken', 'cleartoken');
                        echo "<p><a href='{$url}' class='button button-secondary'>Revoke Access</a></p>";
                    }
                    // Was there an error?
                    else {

                        if(!empty($settings['error'])) {
                            echo '<div class="error inline clear">'.wpautop( $settings['error'] ).'</div>';
                        }
                        ...
                        $url = $salesforceService->getAuthorizationUri(array(
                            ...
                            'state' => admin_url('admin.php?page=gf_settings&addon=Salesforce')
                        ));

                        echo sprintf("<p><a href='%s' class='button button-primary button-hero'>%s</a></p>", $url, __('Login with Salesforce!', 'gravity-forms-salesforce'));
                    }
                    ?>
                    </td>
                </tr>
    ...
    public static function get_api($settings = array()){
        ...
        $libpath = plugin_dir_path(__FILE__).'lib/Force.com-Toolkit-for-PHP/soapclient/';
        ...
        try {
            ...
            # self::r($auth, false, '$auth from getAccessToken');
            ...
            */
            ...
            // Set the session ID as the auth code
            $mySforceConnection->setSessionHeader($auth);

            // Get the endpoint from the session ID
            // @see http://blogs.developerforce.com/developer-relations/2011/03/oauth-and-the-soap-api.html
            $mySforceConnection->setEndpoint(self::getEndpoint($auth));

            $mySforceConnection = apply_filters('gf_salesforce_connection', $mySforceConnection);

            // TODO: this is just a test.
            #$mySforceConnection->describeSObject('account');

            # self::r($mySforceConnection, false, '$mySforceConnection');
    ...
    static private function getSalesforceService() {
        ...
        $credentials = new Credentials(
            '3MVG9y6x0357HledYGqazOFNBPUUAVtD9yRYNGk0TAknMvch_rM8aZxzmtd6T7lcBTB4mPQMJ9VZsBqF8kfBX', //$consumerId,
            '7911901779233377323', //$consumerSecret,
            'https://katz.co/oauth/' //$currentUri->getAbsoluteUri() //$callbackUrl
        );

        $client = new CurlClient;
        $Storage = new WordPressMemory;
        $scopes = array('api', 'refresh_token');
        $salesforceService = new SalesforceService($credentials, $client, $Storage, $scopes);
    ...
    static function getEndpoint($auth = '') {
        ...
        $request = wp_remote_get( add_query_arg(array('oauth_token' => $auth), $id ));

        if(!is_wp_error( $request ) && (int)$request['response']['code'] === 200) {
            $response = $request['body'];
            $response = json_decode($response);
            if(is_object($response)) {
                return str_replace('{version}', '27.0', $response->urls->partner);
            }
        }
    ...
    static private function getToken() {
        $Storage = new WordPressMemory;
        if($Storage->hasAccessToken('SalesforceService')) {
            return $Storage->retrieveAccessToken('SalesforceService');
        }
        return false;
    }
    ...
    static private function clearToken() {
        $Storage = new WordPressMemory;
        $Storage->clearToken('SalesforceService');
    }
    ...
    public function r($content, $die = false, $title = '') {
        if(!empty($title)) {
            echo '<h3>'.$title.'</h3>';
        }
        echo '<pre>';
